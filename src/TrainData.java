...
public class TrainData
{
	...
	protected void addAttributeValues( int attrIndex, int[] val )
	{
		Attribute attr = this.attribute_list.get( attrIndex );
		for ( int i = 0; i < val.length; i++ )
			attr.values[ i ] = val[ i ];
	}

	protected void addClassAttributeValues( Attribute classAttr )
	{
		for ( int i = 0; i < classAttr.values.length; i++ )
			this.class_data.values[ i ] = classAttr.values[ i ];
	}
	...
	protected void copyTrainData( int i, int val )
	{
		this.attribute_list.get( i ).attribute_data.add( val );
	}
	...
	protected void copyClassTrainData( int val )
	{
		this.class_data.attribute_data.add( val );
	}
	...
	protected void calculateAccuracyOfTestFile( TreeData tree, String test_file_name ) throws FileNotFoundException
	{
		try
		{
			...
			DecimalFormat df = new DecimalFormat( "#.##" );
			System.out.println( "Accuracy of test file with train data ( " + i + " instances ) = " + df.format( matched_count * 100.00 / i ) );
			in.close();
		}
		catch ( FileNotFoundException e )
		{
	...
	protected void calculateAccuracyOfTestFile( ArrayList< TreeData > tree, String test_file_name ) throws FileNotFoundException
	{
		try
		{
			File test_file = new File( test_file_name );
			...
			while ( in.hasNextLine() )
			{
				int[] class_voting = new int[ this.class_data.value_count ];
				i++;
				tokens = in.nextLine().trim().split( "\\s+" );
					...
					while ( tempTree.attribute != null )
					{
						...
						for ( Map.Entry< Integer, TreeData > entry : tempTree.childList.entrySet() )
						{
							if ( val == entry.getKey() )
							{
								...
								{
									found = true;
									int class_val = entry.getValue().class_value;
									class_voting[ class_val ] += 1;
									break;
								}
							}
						}
				...
				for ( int j = 0; j < class_voting.length; j++ )
				{
					if ( class_voting[ j ] > max )
					{
						max = class_voting[ j ];
						index = j;
					}
			...
			DecimalFormat df = new DecimalFormat( "#.##" );
			System.out.println( "Accuracy of test file with bag data ( " + i + " instances, " + tree.size() + " bags  ) = " + df.format( matched_count * 100.00 / i ) );
			in.close();
		}
		catch ( FileNotFoundException e )
		{
